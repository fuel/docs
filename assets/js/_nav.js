...
function show_nav(page, path)
{
	...
	$.each(panes, function(pane, subjects){
		var _subjects = [];
		var $pane = $('<div id="pane_'+pane+'" class="table"></div>');
		
		var $column = $('<ul class="column"></ul>');
		$.each(subjects, function(subject, content){
			if(content == '--')
			{
				$pane.append($subject);
				$subject = $('<ul class="column"></ul>');
			}
			else
			{
				if(_subjects[subject] == true)
				{
					subject = '';
				}
				else
				{
					subject = '<h2 class="topic">'+subject+'</h2>';
				}
				$column.append('<li>'+subject+'</li>');
				
				content = generate_nav(path, content);
				if(content.length > 0)
				{
					var $ul = $('<ul></ul>');
					$ul.html(content);
					if($ul.find('.active_link'))
					{
						$pane.addClass('active_pane');
					}
					$column.find('> li').last().append($ul);
				}
			}
		});
		$pane.append($column);
...
var dummy_header = document.createElement('header');
var dummy_nav = document.createElement('nav');
